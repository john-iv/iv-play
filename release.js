/* release.js — read by index.html and IV/Play's "Check for Updates" (F1).
   Bump per release; buildId is the field that drives the update check. */
var IVPLAY = {
    version:    "2.8.12",
    buildId:    "54442eb7-ff78-420b-a164-9acc898d6c83",
    date:       "11:13 PM 8/31/2026",
    url:        "https://github.com/john-iv/iv-play/releases",
    artMameVer: "0.289",
    artDate:    "5:54 PM 7/30/2026",
    notes: [
        "DPI & Layout Modernization",
        "Internal HiScore.ini",
        "MAME Source .cpp Peek",
        "TohoScope Int/Ext Toggle",
        "Live Available ROM Monitoring",
        "Favorite.ini External Monitoring",
        "Persistent DAT View via Tilde",
        "Check for Update button on F1",
        "-Bench Commandline Switch",
        "Tweaks and bugfixes 2.8.12",
        "See User Guide for Full ChangeLog"
    ]
};